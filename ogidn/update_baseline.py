...
Runs the OG-IDN ``Calibration`` with ``update_from_api=True`` and writes the
overlay back into ``ogidn_default_parameters.json`` so an offline run
(``update_from_api=False``) reproduces the connected calibration. The overlay
is the live data OG-IDN sources from APIs:
...
unchanged; see ``ogidn.macro_params`` and the calibration docs.

Usage::

    uv run python -m ogidn.update_baseline
"""
...
JSON_PATH = os.path.join(
    os.path.dirname(__file__), "ogidn_default_parameters.json"
)
